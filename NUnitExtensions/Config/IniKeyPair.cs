namespace NUnit.Framework.Config
{
    ...
    public abstract class IniKeyPair<T> : IDictionary<string, T>
    {
        ...
        {
            if (item.Key == null) throw new ArgumentException("Key may not be null", nameof(item));
            if (item.Value == null) throw new ArgumentException("Value may not be null", nameof(item));
